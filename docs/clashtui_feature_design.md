...
-   为每个 proxy-provider 生成一个 proxy-group:
    ...
    ```yaml
    - name: "At"
      expand_group_with: ["${pvd}"] # 也可以写多个 proxy-provider name, e.g. ["${pvd::pvd0}", "${pvd::pvd2}"]
      type: url-test
      <<: *pa_dt
    ```
  ...
  "outbounds": [
    {
      "type": "urltest",
      "tag": "auto-proxy",
      "expand_outbound_with": ["${pvd}"], // 也可以写多个 proxy-provider name, e.g. ["${pvd::pvd0}", "${pvd::pvd2}"]
...
```json
{
  ...
  "outbounds": [
    ...
    // 如果用 `"${pvd}"` 会扩展为 `Auto-pvd0, Auto-pvd1, ...`
    {
      "type": "urltest",
      "tag": "Auto",
      "expand_group_with": ["${pvd}"], // OR `"expand_group_with": ["auto-proxy", "${pvd::pvd0}", "${pvd::pvd2}"],`
      "url": "https://www.gstatic.com/generate_204",
      "interval": "5m",
      "tolerance": 50
    },
