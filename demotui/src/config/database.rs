...
impl ProfileType {
    ...
    pub fn get_domain(&self) -> Option<String> {
        match self {
            ...
            ProfileType::Url(url) => extract_domain(url),
            ProfileType::Generated(name) => Some(format!("From template {name}")),
            ProfileType::Github { url, token: _ } => extract_domain(url),
            ProfileType::GitLab { url, token: _ } => extract_domain(url),
        }
    }
}
